...
// CoreDump returns true if ws indicates that a core dump was produced,
// consistent with WCOREDUMP.
//
// Preconditions: ws.Signaled().
func (ws WaitStatus) CoreDump() bool {
	return ws&0x80 != 0
}
...
// TrapCause returns the PTRACE_EVENT_* field in ws.
//
// Preconditions: ws.Stopped().
func (ws WaitStatus) TrapCause() uint32 {
	return uint32(ws >> 16)
}
...
func (ws WaitStatus) String() string {
	...
	case ws.Stopped():
		if ptraceEvent := ws.TrapCause(); ptraceEvent != 0 {
			return fmt.Sprintf("stopped by signal %d (PTRACE_EVENT %d)", ws.StopSignal(), ptraceEvent)
		}
